...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT b FROM Booking b, Item i WHERE b.itemId = i.id " +
            "AND i.ownerId = :ownerId AND b.start < :now AND b.end > :now")
    ...
    @Query("SELECT b FROM Booking b, Item i WHERE b.itemId = i.id " +
            "AND b.itemId = :itemId AND b.bookerId = :bookerId AND b.end < :now AND b.status = 'APPROVED'")
    List<Booking> findCompletedBookingsForItemAndUser(
    ...
    @Query("SELECT COUNT(b) > 0 FROM Booking b, Item i WHERE b.itemId = i.id " +
            "AND b.itemId = :itemId AND b.bookerId = :bookerId AND b.end < :now AND b.status = 'APPROVED'")
